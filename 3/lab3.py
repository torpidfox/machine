...
def draw_data(df, marker, label):
	...
	plt.scatter(red[df.columns[0]], red[df.columns[1]], color='r', marker=marker, label=label)

	green = df.loc[df[df.columns[-1]] == 'green']
	plt.scatter(green[df.columns[0]], green[df.columns[1]], color='g', marker=marker, label=label)
...
def test_params(task_num, c=1):
	...
	gs_test.fit(train_df[train_df.columns[:-1]], train_df[train_df.columns[-1]])

	return gs_train.cv_results_, gs_test.cv_results_


def perform_task(task_num, c=1, kernel='linear'):
	with open('data/svmdata{}.txt'.format(task_num)) as f:
		train_df = pd.read_csv(f, sep='\t')

	clf = svm.SVC(random_state=0, tol=1e-5, C=c, kernel=kernel)
	clf.fit(train_df[train_df.columns[:-1]], train_df[train_df.columns[-1]])

	with open('data/svmdata{}test.txt'.format(task_num)) as f:
		test_df = pd.read_csv(f, sep='\t')

	acc_test = clf.score(test_df[test_df.columns[:-1]], test_df[test_df.columns[-1]])
	acc_train = clf.score(train_df[test_df.columns[:-1]], train_df[test_df.columns[-1]])
	...
	if kernel == 'linear':
		...
		plt.legend()
		plt.title('Линейное ядро, c={}, точность на тестовой выборке - {}, точность на обучающей выборке - {}, количество опорных векторов - {}'.format(c, acc_test, acc_train, clf.n_support_))
